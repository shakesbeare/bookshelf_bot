...
impl Database {
    ...
    pub async fn monthly_leaderboard(&self) -> Result<Vec<LeaderboardEntry>> {
        let mut leaderboard: Vec<LeaderboardEntry> = sqlx::query_as(
            ...
            SELECT DISTINCT users.username, (
                SELECT COUNT(*) FROM user_books_read
                WHERE user_id = users.id AND date(user_books_read.datetime) >= date('now', '-24 hours', 'start of month', '+16 hours')
            ) AS books_read
            FROM users
            INNER JOIN user_books_read ON users.id = user_books_read.user_id
            WHERE date(user_books_read.datetime) >= date('now', '-24 hours', 'start of month', '+16 hours');
            "#,
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn yearly_leaderboard(&self) -> Result<Vec<LeaderboardEntry>> {
        let mut leaderboard: Vec<LeaderboardEntry> = sqlx::query_as(
            ...
            SELECT DISTINCT users.username, (
                SELECT COUNT(*) FROM user_books_read
                WHERE user_id = users.id AND date(user_books_read.datetime) >= date('now', '-24 hours', 'start of year', '+16 hours')
            ) AS books_read
            FROM users
            INNER JOIN user_books_read ON users.id = user_books_read.user_id
            WHERE date(user_books_read.datetime) >= date('now', '-24 hours', 'start of year', '+16 hours');
            "#,
        )
        .fetch_all(&self.pool)
        .await?;
